...
The Coder Team template's planning + implementation flow is
...
`WORKSPACE_ROOT` is unset, falls back to a bare `isdir` check
(legacy `CODING_REPO_PATH`-only deployments aren't broken by
upgrading) — but that's weaker, so production deployments should
always set it.
...
- **Per-tool keys.** All routes share `ANOTHER_CODER_API_KEY`. The
  granularity buys little for a single-operator deployment; tracked
  in TASKS.md as a "later if a real user asks" item.
