"""Coverage for the Phase 4 additions:

  1. Structured ``error_kind`` on JobManager.mark_failed surfaces
     through to.to_status_response. Locks the wire contract that the
     platform's poller can branch on (forward-compatible — no
     platform change required today).
...
schema tests + the existing route tests; a dedicated runner test
would need a real subprocess mock orchestration that's out of
scope here.
"""
