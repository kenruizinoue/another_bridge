...
def test_prune_older_than_removes_stale_rows_only(tmp_path) -> None:
    """The TTL reaper calls this on a timer with a 7-day cutoff.
    Verify a row's last_seen_at gets updated on every set_session
    so a recently-touched conversation isn't pruned just because
    its row was originally created weeks ago."""
    store = SessionStore(db_path=str(tmp_path / "sessions.db"))
