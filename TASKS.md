...
- [ ] **Enrich `/health` further with claude-probe state + session-store reachable.** Lifespan already runs `probe_claude_binary()` at boot and logs the result, but it doesn't persist into `app.state` for `/health` to read. Add `app.state.claude_probe = {"ok": ..., "detail": ...}` in lifespan, then surface it on `/health` alongside a cheap `SELECT 1` against `session_store` to verify SQLite reachability. Useful for bug triage ("is your claude binary OK?") without a separate diagnostic endpoint. Deferred from the version-bump pass — version alone closes the most common bug-report question.
